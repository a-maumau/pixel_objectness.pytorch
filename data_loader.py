...
class VOC12_Segmentation(_segmentation):
	def __init__(self, file_list_path, img_root, mask_root, pair_transform=None, input_transform=None, target_transform=None):
		...
		self.images = [self.images[i].rstrip("\n") for i in range(len(self.images))]
		...
		for i in range(self.data_num):
			...
			_img = np.asarray(Image.open(os.path.join(self.img_root,self.images[i]["file_name"])).convert('RGB')) # not thinking there is a empty input...
			...
			_mask_img = np.asarray(Image.open(os.path.join(self.mask_root,re.sub(r'.jpg', "",self.images[i]["file_name"])+".png")).convert('P'))
